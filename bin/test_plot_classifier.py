...
def visualize_and_annotate_sen(df, metric, output_directory):
    ...
    ax = sns.boxplot(x="basecaller", y=metric, hue="classifier", data=df, palette="Set2", order=order)
    
    # Overlay jittered strip plots
    sns.stripplot(x="basecaller", y=metric, hue="classifier", data=df, dodge=True, jitter=True, size=5, alpha=0.5, ax=ax, marker="D", edgecolor="gray", linewidth=0.5, order=order)
    
    # Adjust title, axis labels, and formatting
    ax.set_title(f'{metric.capitalize()} Distribution by Basecaller for each Metagenomic Classifier', fontsize=28)
