        ...
        for line in file:
            ...
            if len(fields) >= 3:
                ...
                if taxon_level in taxon_levels:
                    ...
                    classified = 100.0
                taxon_levels['U'] += classified

    return taxon_levels

def save_to_csv(out_file, read_lengths, kraken2_percentages, braken_percentages, cent_percentages):
    kraken2_unclassified = [100 - x for x in kraken2_percentages]
    braken_unclassified = [100 - x for x in braken_percentages]
    cent_unclassified = [100 - x for x in cent_percentages]
    
    with open(out_file, 'w') as file:
        writer = csv.writer(file)
        writer.writerow(["read length", 
                         "percentage classified (Kraken2)", "percentage unclassified (Kraken2)", 
                         "percentage classified (braken)", "percentage unclassified (braken)", 
                         "percentage classified (Centrifuge)", "percentage unclassified (Centrifuge)"])
        
        for i in range(len(read_lengths)):
            writer.writerow([read_lengths[i], 
                             kraken2_percentages[i], kraken2_unclassified[i], 
                             braken_percentages[i], braken_unclassified[i], 
                             cent_percentages[i], cent_unclassified[i]])
...
# plt.show()
save_to_csv(outdir+'/classification_percentages.csv', read_lengths, kraken2_taxon_levels, braken_taxon_levels, cent_no_rank_percentage_data)