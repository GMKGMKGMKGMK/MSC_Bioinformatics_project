...
if __name__ == "__main__":
    ...
    # Group by classifier and calculate the mean for sensitivity, precision, and F1
    grouped = df.groupby('classifier').mean()

    # Calculate F1 score
    grouped['f1'] = 2 * (grouped['precision'] * grouped['sensitivity']) / (grouped['precision'] + grouped['sensitivity'])

    # Plotting average sensitivity for each classifier
    plt.figure(figsize=(10, 6))
    ax_sensitivity = grouped['sensitivity'].plot(kind='bar', color='skyblue')
    annotate_bars(ax_sensitivity, grouped['sensitivity'])
    plt.yticks(np.arange(0, 1.1, 0.1))
    plt.title('Average Sensitivity by Metagenomic Classifier')
    ...
    # Plotting average F1 score for each classifier
    plt.figure(figsize=(10, 6))
